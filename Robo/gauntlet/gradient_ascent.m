function [positions, headings] = gradient_ascent(grad, pos_0, heading_0, lambda_0, delta, tolerance, max_steps)
    ...
    while i < max_steps
        ...
        gradient = grad(positions(i - 1, 1), positions(i - 1, 2))';
        positions(i, :) = positions(i - 1, :) + (lambdas(i - 1) .* gradient);
        
        headings(i, :) = gradient ./ norm(gradient);
        
        %% Exit Condition
        if abs(vecnorm(positions(i, :) - positions(i - 1, :))) < tolerance
            break
        end
        
        %% Expand lists as needed
        if i == i_max
            positions = [positions; zeros(i_max, 2)];
